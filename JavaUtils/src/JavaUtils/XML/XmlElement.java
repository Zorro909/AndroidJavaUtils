...
public class XmlElement implements ParseAble, Section {
	...
	public XmlElement(String name, LinkedHashMap<String, String> attributes, String xml, XmlParserEngine engine) {
		this.name = name;
		this.attributes = attributes;
		encode(xml, engine);
	}
	...
	public XmlElement(String name, HashMap<String, Object> objects, LinkedHashMap<String, String> atributes) {
		this.name = name;
	...
	private boolean getOption(EngineOption option, HashMap<EngineOption, Boolean> options, boolean standard) {
	...
	private String checkForXmlDeclarations(String xml, String... declarationstarts) {
		for (int i = 0; i < declarationstarts.length; i = i + 2) {
			if (xml.startsWith(declarationstarts[i]))
				xml = xml.substring(xml.indexOf(declarationstarts[i + 1]) + declarationstarts[i].length());
		}
		return xml;
	}
	...
	private void _encode_recommended(String xml, XmlParserEngine engine, boolean startNewThreads, int deep) {
			...
			if (value) {
				...
				if (startNewThreads && deep == 2) {
					if (ThreadManager.ifCreateNewThread()) {
						RecommendedParserRunnable rc = new RecommendedParserRunnable(xml.substring(0, end), engine,
								this, false, nextName, attributes, deep + 1);
	...
	private void encode_old(String xml, XmlParserEngine engine) {
		...
		if (xml.contains("<")) {
			while (xml.contains("<")) {
				...
				String b = xml;
				boolean plus = false;
				if (b.indexOf("</" + nextName + ">") > b.indexOf("<" + nextName) && b.indexOf("<" + nextName) > -1) {
					while (b.indexOf("</" + nextName + ">") > b.indexOf("<" + nextName)
							&& b.indexOf("<" + nextName) > -1) {
						end = end + b.indexOf("</" + nextName + ">") + ("</" + nextName + ">").length();
						b = b.substring(b.indexOf("</" + nextName + ">") + ("</" + nextName + ">").length());
				...
				if (attributes.containsKey("type") && attributes.get("type").contains("ParseAble")) {
					String type = attributes.get("type").split("--")[1];
					if (values.containsKey(nextName)) {
						LinkedList<Object> ele = values.get(nextName);
						try {
							ele.add(ParserManager.getParseAble(type).fromParser(new XmlElement(nextName, attributes,
						...
						try {
							...
							XmlElement xm = new XmlElement(nextName, attributes, xml.substring(0, end),
	...
	public LinkedList<XmlElement> getSections(String name) throws NoElementFoundException {
		if (values != null) {
			if (!values.containsKey(name))
				throw new NoElementFoundException("Xml Section", name);
			...
			try {
				for (LinkedList<XmlElement> a : listSections().values()) {
					for (XmlElement x : a) {
						...
						if (!x.value.isEmpty()) {
							if (!x.value.equals("\\\\\\\\")) {
								element = element + x.decode(2) + "</" + x.name + ">" + line_seperator;
							}
						} else {
							element = element + line_seperator + x.decode(2) + line_seperator + "</" + x.name + ">"
									+ line_seperator;
						}
						xml = xml + element;
					}
	...
	public ParseAble fromParser(Section p) throws ParseException {
		if (p instanceof XmlElement)
			return new XmlElement((XmlElement) p);
		return null;
	}
	...
	class RecommendedParserRunnable implements Runnable {
		...
		public RecommendedParserRunnable(String xml, XmlParserEngine engine, XmlElement ele, boolean startNewThreads,
				int deep) {
			this.xml = xml;
			element = ele;
			eng = engine;
		...
		public RecommendedParserRunnable(String xml, XmlParserEngine engine, XmlElement ele, boolean startNewThreads,
				String nextName, LinkedHashMap<String, String> attributes, int deep) {
			this.xml = xml;
			element = ele;
			eng = engine;
			this.startNewThreads = startNewThreads;
